...
exports.DataSheetGrid = react_1.default.memo(react_1.default.forwardRef(({ value: data = DEFAULT_DATA, className, style, height: maxHeight = 400, onChange = DEFAULT_EMPTY_CALLBACK, columns: rawColumns = DEFAULT_COLUMNS, rowHeight = 40, headerRowHeight = typeof rowHeight === 'number' ? rowHeight : 40, gutterColumn, stickyRightColumn, rowKey, addRowsComponent: AddRowsComponent = AddRows_1.AddRows, createRow = DEFAULT_CREATE_ROW, autoAddRow = false, lockRows = false, disableExpandSelection = false, duplicateRow = DEFAULT_DUPLICATE_ROW, contextMenuComponent: ContextMenuComponent = ContextMenu_1.ContextMenu, disableContextMenu: disableContextMenuRaw = false, onFocus = DEFAULT_EMPTY_CALLBACK, onBlur = DEFAULT_EMPTY_CALLBACK, onActiveCellChange = DEFAULT_EMPTY_CALLBACK, onSelectionChange = DEFAULT_EMPTY_CALLBACK, rowClassName, cellClassName, onScroll, }, ref) => {
    ...
    const [activeHeaderCell, setActiveHeaderCell] = (0, useDeepEqualState_1.useDeepEqualState)(null);
    ...
    const applyPasteDataToDatasheet = (0, react_1.useCallback)((pasteData, options = {}) => __awaiter(void 0, void 0, void 0, function* () {
        var _j;
        if (lockRows && options.insertBelow) {
            return;
        }
        const updateDataRows = (data_, pasteData_, firstPastedCell_) => __awaiter(void 0, void 0, void 0, function* () {
            var _k;
            for (let columnIndex = 0; columnIndex < pasteData_[0].length &&
                firstPastedCell_.col + columnIndex <
                    columns.length - (hasStickyRightColumn ? 2 : 1); columnIndex++) {
                const pasteValue = (_k = columns[firstPastedCell_.col + columnIndex + 1]) === null || _k === void 0 ? void 0 : _k.pasteValue;
                if (pasteValue) {
                    for (let rowIndex = 0; rowIndex < pasteData_.length; rowIndex++) {
                        if (!isCellDisabled({
                            col: firstPastedCell_.col + columnIndex,
                            row: firstPastedCell_.row + rowIndex,
                        })) {
                            data_[firstPastedCell_.row + rowIndex] = yield pasteValue({
        ...
        if (options.insertBelow) {
            ...
            yield updateDataRows(newData, pasteData, firstPastedCell);
            onChange(newData, [
                {
                    type: 'CREATE',
            ...
            setSelectionCell({
                col: Math.min(firstPastedCell.col + pasteData[0].length - 1, columns.length - (hasStickyRightColumn ? 3 : 2)),
                row: lastPastedRow,
            });
            return;
        }
        ...
        if (!activeHeaderCell && (!editing && activeCell)) { // Cannot paste to header cell or data cells being edited.
            ...
            if (pasteData.length === 1) {
                ...
                yield updateDataRows(newData, pasteData, min);
                const operations = [
                    {
                        type: 'UPDATE',
                        fromRowIndex: min.row,
    ...
    const paste = (0, react_1.useCallback)((options = {}) => __awaiter(void 0, void 0, void 0, function* () {
        if (navigator.clipboard.read !== undefined) {
            ...
            applyPasteDataToDatasheet((0, copyPasting_1.parseTextPlainData)(text), options);
        }
        else {
    ...
    const onMouseDown = (0, react_1.useCallback)((event) => {
        ...
        const rightClick = event.button === 2 || (event.button === 0 && event.ctrlKey); // right button OR left + CTRL
        ...
        const clickOnSelectedStickyRightColumn = clickOnStickyRightColumn &&
            ...
            cursorIndex.row >= selection.min.row &&
            cursorIndex.row <= selection.max.row;
        const clickOnHeaderCell = (cursorIndex === null || cursorIndex === void 0 ? void 0 : cursorIndex.row) === -1 &&
            (cursorIndex === null || cursorIndex === void 0 ? void 0 : cursorIndex.col) >= 0 &&
        ...
        if (clickOnHeaderCell) {
            setActiveHeaderCell(data.length === 0
        ...
        if ((!(event.shiftKey && activeCell) || rightClick) && // When shift is pressed, dont change active cell. Expand the selection.
            data.length > 0) {
            setActiveCell(cursorIndex && {
                col: (rightClickInSelection || rightClickOnSelectedHeaders) &&
                    ...
                    : Math.max(// 
                    0, clickOnStickyRightColumn ? 0 : cursorIndex.col),
    ...
    const onKeyDown = (0, react_1.useCallback)((event) => {
        if (activeCell) {
            ...
            else if ((event.key.match(/^[ -~]$/) || event.code.match(/Key[A-Z]$/)) &&
                !event.ctrlKey &&
                !event.metaKey &&
                !event.altKey) {
    ...
    (0, react_1.useEffect)(() => {
        ...
        if (activeHeaderCell || (activeCell === null || activeCell === void 0 ? void 0 : activeCell.row) !== undefined) {
            items.push({
                type: 'PASTE_INSERT_BELOW',
                action: () => __awaiter(void 0, void 0, void 0, function* () {
                    yield paste({ insertBelow: true });
                    setContextMenu(null);
                }),
            });
        }
    ...
    return (react_1.default.createElement("div", { className: className, style: style },
        ...
        react_1.default.createElement(Grid_1.Grid, { columns: columns, outerRef: outerRef, columnWidths: columnWidths, hasStickyRightColumn: hasStickyRightColumn, displayHeight: displayHeight, data: data, fullWidth: fullWidth, headerRowHeight: headerRowHeight, activeHeaderCell: activeHeaderCell, activeCell: activeCell, innerRef: innerRef, rowHeight: getRowSize, rowKey: rowKey, selection: selection, rowClassName: rowClassName, editing: editing, getContextMenuItems: getContextMenuItems, setRowData: setRowData, deleteRows: deleteRows, insertRowAfter: insertRowAfter, duplicateRows: duplicateRows, stopEditing: stopEditing, cellClassName: cellClassName, onScroll: onScroll },
